...
### Checklist
- [x] Добавить замеры времени (логами/таймстампами):
  - [x] import/open (до первого кадра контента)
  - [x] unzip + parse OPF/TOC
  - [x] build chapters (извлечение текста)
- [x] Добавить возможность включать/выключать perf-логи (например, флаг `kDebugMode`).
- [x] Прогнать 2–3 “тяжёлых” EPUB (большой объём текста, много глав).
- [x] Зафиксировать решение: **chapters-as-pages** или **chunked virtual list**, с аргументами.
...
### Checklist
- [x] Перейти на отображение **по главам** (каждая глава — отдельная сущность рендера).
- [x] Навигация по TOC открывает конкретную главу (без пересборки всей книги).
- [x] Сохранить текущие функции: TOC, переходы, восстановление позиции (где возможно).
- [x] Не ломать Desktop split-view (если он используется).
...
### Checklist
- [x] Добавить кеш (на время сессии чтения) для текста глав или промежуточных результатов.
- [x] Инвалидировать кеш при закрытии книги/смене книги.
- [x] Добавить простые логи “cache hit/miss” в debug.
...
### Checklist
- [x] Ввести единое state-машину: `loading` → `content` → `error`.
- [x] Показать UI для `error` с кнопкой `Retry`.
- [x] Обработать типовые ошибки: файл удалён, нет доступа, zip битый, таймаут.
...
### Checklist
- [x] Создать `ReaderController` (или аналог) с публичным state.
- [x] Перенести из Widget: загрузку книги, парсинг, подготовку глав, обработку ошибок.
- [x] Обновить Reader UI: подписка на state + вызов действий контроллера.
- [x] Покрыть контроллер unit-тестом на state transitions (минимум).
...
### Checklist
- [x] Добавить режим `Grid` для библиотеки (крупная обложка + название).
- [x] Сохранить текущий список как дефолтный режим.
- [x] Добавить переключатель `Список / Плитка`.
- [x] Запоминать выбор режима (локально).
