...
## AC5 — Синхронизация заметок/выделений/закладок между устройствами

1) На устройстве A открой книгу и создай:
   - 1 заметку (note),
   - 1 выделение (highlight),
   - 1 закладку (bookmark).
2) Перейди в `Заметки` и создай 1 free note (без книги).
3) Нажми `Синхронизировать сейчас`.
4) На устройстве B подключи тот же провайдер и синхронизируй.

Ожидаемо:
- Все созданные элементы появились на устройстве B.
- Открытие note/highlight/закладки из списка приводит к правильному месту в Reader.
- Free note доступна в списке `Заметки`.
