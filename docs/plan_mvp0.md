...
Чеклист:
- [ ] Привести `assets/` в порядок: создать папку `assets/` **или** убрать `assets:` из `pubspec.yaml` до появления ресурсов.
- [ ] Привести `README.md` и `STATE.md` в соответствие фактической структуре.
- [ ] В `docs/spec_mvp.md` разделить scope на **MVP0 / MVP1 / MVP2**.
- [ ] В `docs/dod.md` уточнить DoD для PR (линт/тесты/ручная проверка/доки).
...
Чеклист:
- [ ] Зафиксировать в `docs/decisions.md` выбор:
  - file picker (desktop/macOS),
  - пути хранения (app-managed storage),
  - epub parsing (минимум метаданные),
  - локальное хранилище (DB/kv).
- [ ] Подключить зависимости в `pubspec.yaml`.
- [ ] Ввести интерфейсы: `StorageService`, `BookRepository`, модели (Book + DTO/record).
...
Чеклист:
- [ ] Реальный выбор файла EPUB из LibraryScreen.
- [ ] Корректная обработка cancel.
- [ ] Валидация: расширение/существование/читаемость.
- [ ] Показ ошибок пользователю (snackbar/dialog).
...
Чеклист:
- [ ] Копирование выбранного EPUB в app-managed storage.
- [ ] Конфликты имён: безопасное переименование (суффикс/хэш).
- [ ] Атомарность: temp → rename (по возможности).
...
Чеклист:
- [ ] Извлечь `title` (обязательно), `author` (желательно).
- [ ] Fallback: имя файла, если метаданные отсутствуют.
...
Чеклист:
- [ ] Сохранение списка книг (id, title, author, localPath, addedAt).
- [ ] Загрузка при старте.
- [ ] Удаление книги: удалить запись + файл.
