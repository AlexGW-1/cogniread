...
## Синхронизация
- Этап 1: синхронизация через облачные диски/NAS (Dropbox, Яндекс.Диск, WebDAV/SMB) с единым клиентским адаптером. Google Drive/OneDrive — отложены.
- Этап 2 (future): WebSockets + собственный sync‑gateway — реалтайм‑канал синхронизации между устройствами без изменения доменной модели.
