...
Факты по коду (на 2026‑01‑18):
- Notes/Highlights/Bookmarks реализованы.
- Поиск по книге реализован.
- Импорт и storage поддерживают несколько форматов (см. `STATE.md`).
- Server-less file-based синхронизация реализована (Dropbox, Yandex Disk, WebDAV/SMB).
...
### Checklist
- [x] Зафиксировать решение (коротко в `docs/`):
...
- [x] Под выбранный вариант:
  - [x] удалить/пометить deprecated неиспользуемые папки/классы, **или**
  - [x] подключить 1 реальный usecase (без изменения UI) и удалить дубли.
